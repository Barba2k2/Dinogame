...
sprite_sheet = pygame.image.load(os.path.join(diretorio_imagens, 'dinoSpritesheet.gif')).convert_alpha()
print("Sprite Sheet dimensions:", sprite_sheet.get_size())
...
class Dino(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Load each frame of the dino animation
        for i in range(1, 4):  # Assuming you have 3 frames for the dino
            img = pygame.image.load(os.path.join(diretorio_imagens, f'frame-{i}.gif')).convert_alpha()
            img = pygame.transform.scale(img, (32*3, 32*3))  # Scale the image if needed
        ...
        self.pos_y_inicial = ALTURA - 64 - self.image.get_height()//2
...
class Nuvens(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # Calculate the width of a single cloud if there are 6 clouds in the strip
        cloud_width = cloud_strip.get_width() // 6  # 348 // 6 = 58
        cloud_height = cloud_strip.get_height()  # Should be 30 as mentioned
        
        # Select a random cloud from the strip
        cloud_position = randrange(6) * cloud_width
        
        # Now we can extract a single cloud from the strip using subsurface
